...
export default function CourseForm({ initialCourse, onSave, onCancel }: CourseFormProps) {
    ...
    // 6  return ส่วนแสดงผล
    // กลุ่มฟอร์มเพิ่มและแก้ไขรายวิชา
    return (
        <form className="courseForm" onSubmit={handleSubmit} noValidate>
            {/* จัดแต่ละช่องข้อมูลให้เรียงเป็นคอลัมน์ในแถบแนวนอน */}
            ...
            {/* จัดกลุ่มปุ่มบันทึกและยกเลิกให้อยู่ในแถวเดียวกัน */}
            <div className="courseFormActions">
            <button className="courseActionButton courseActionButton--save" type="submit">บันทึก</button>
            {initialCourse ? (
                <button className="courseActionButton" type="button" onClick={onCancel}>ยกเลิก</button>
            ) : null}
            </div>
        </form>   
